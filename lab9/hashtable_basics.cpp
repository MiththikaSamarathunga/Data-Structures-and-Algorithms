...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        //take the sum of all the ASCII values of the each character in user_name 
        for (char character : user_name){
            sum += (int)character;
        }
    ...
    void insert(string user_name,string user_password){
        int hash;
        bool empty;
        hash = hashfunc(user_name);// take the hashcode 
        empty = is_slot_empty(hash);
	//add an if condition to complete the code here
        if (empty){
            password[hash] = user_password;
            cout<<"Your password was saved successfully \n";
        }
        else{
            cout<<"A collision occured. Working to resolve the problem.\n";
